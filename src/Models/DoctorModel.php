...
class DoctorModel
{
    ...
    // =================================================================
    // 🤖 الإقفال التلقائي للزيارات في نهاية اليوم (24 ساعة)
    // =================================================================
    //
    // المنطق (محدّث):
    //   - أي زيارة status='Active' لم يُغلقها الطبيب يُغلقها النظام تلقائياً
    //     عند نهاية اليوم (24 ساعة كاملة) — وليس عند نهاية الفترة (shift).
    //   - بمعنى آخر: الزيارة تبقى مفتوحة حتى لو انتهت الفترة الصباحية
    //     ودخلت الفترة المسائية. لا تُغلق إلا عندما:
    //       (أ) يمر تاريخ إنشائها (created_at::date < CURRENT_DATE)
    //           — أي بدأ يوم تقويمي جديد، أو
    //       (ب) مضى 24 ساعة كاملة على إنشائها (احتياطي للزيارات التي
    //           أُنشئت قرب منتصف الليل والنظام لم يفحصها فوراً).
    //   - الزيارة تُحدَّث إلى:
    //       diagnosis      = 'null'
    //       final_notes    = 'null'
    ...
    //
    // ملاحظة مهمة عن الفترات (shifts):
    //   • انتهاء الفترة (shift) لم يعد يُسبب إقفال الزيارات. الفترات تُغلق
    //     لأغراض الفواتير/الإيرادات فقط، أما الزيارة فتبقى مفتوحة طوال
    //     اليوم الكامل (24 ساعة) حتى يُغلقها الطبيب يدوياً أو يحل يوم جديد.
    //   • نتجاهل أي قيمة فاضية/null/whitespace-only للتشخيص أو الملاحظات
    //     ونستبدلها بالنص الحرفي 'null' حسب المتطلب.

    /**
     * يجلب قائمة الزيارات النشطة التي يجب إقفالها تلقائياً (مرّ يومها
     * أو مضت 24 ساعة على إنشائها). للقراءة فقط — لا تعديل.
     *
    ...
    public function findExpiredActiveVisits(): array
    {
        ...
        // الافتراضية للخادم (Asia/Aden).
        //
        // قاعدة الإغلاق التلقائي للزيارة (محدّثة):
        //   - تُغلق الزيارة فقط عند نهاية اليوم (24 ساعة) — لا عند نهاية الفترة.
        //   - معيار "نهاية اليوم": تاريخ إنشاء الزيارة أقدم من اليوم الحالي.
        //   - معيار "مرور 24 ساعة" (احتياطي): فرق التوقيت الحالي وإنشاء
        //     الزيارة ≥ يوم واحد (يغطي الزيارات التي أُنشئت قبيل منتصف الليل).
        $sql = "
            SELECT v.visit_id,
                   v.patient_id,
                   v.doctor_id,
            ...
            WHERE v.status = 'Active'
              AND (
                    -- (أ) مضى يوم تقويمي كامل على تاريخ إنشاء الزيارة
                    v.created_at::date < CURRENT_DATE
                 OR -- (ب) مضى 24 ساعة كاملة منذ إنشاء الزيارة (احتياطي)
                    v.created_at <= (CURRENT_TIMESTAMP - INTERVAL '24 hours')
              )
            ORDER BY v.created_at ASC
        ";
        $stmt = $this->conn->prepare($sql);
