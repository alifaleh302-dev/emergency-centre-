...
class FinanceController extends BaseController
{
    ...
    private function getScopedMinistryReport(array $filters): array
    {
        ...
        if ($this->isAdmin()) {
            return $this->model->getMinistryShareReport($dateFilters);
        }

        return $this->getAccountantMinistryReport($dateFilters, $this->userId);
    }

    private function getAccountantMinistryReport(array $filters, int $accountantId): array
    {
        $from = $filters['from'] ?? null;
        $to = $filters['to'] ?? null;

        $invoiceWhere = ' AND i.accountant_id = :accountant_id';
        $invoiceParams = [':accountant_id' => $accountantId];
        if ($from !== null) {
            $invoiceWhere .= ' AND COALESCE(i.paid_at, i.created_at) >= :from';
            $invoiceParams[':from'] = $from;
        }
        if ($to !== null) {
            $invoiceWhere .= ' AND COALESCE(i.paid_at, i.created_at) <= :to';
            $invoiceParams[':to'] = $to;
        }
        ...
        $stmt->execute();
        $byServiceRows = $stmt->fetchAll(PDO::FETCH_ASSOC);

        $ticketShares = $this->model->getTicketMinistryShares();
        $ticketWhere = ' AND t.issued_by = :accountant_id';
        $ticketParams = [':accountant_id' => $accountantId];
        if ($from !== null) {
            $ticketWhere .= ' AND t.created_at >= :from';
            $ticketParams[':from'] = $from;
        }
        if ($to !== null) {
            $ticketWhere .= ' AND t.created_at <= :to';
            $ticketParams[':to'] = $to;
        }
        ...
        $stmt->bindValue(':ms_morning', (float) ($ticketShares['morning'] ?? 30.0));
        $stmt->bindValue(':ms_evening', (float) ($ticketShares['evening'] ?? 100.0));
        $stmt->bindValue(':ms_morning_2', (float) ($ticketShares['morning'] ?? 30.0));
        $stmt->bindValue(':ms_evening_2', (float) ($ticketShares['evening'] ?? 100.0));
        foreach ($ticketParams as $key => $value) {
            $stmt->bindValue($key, $value, is_int($value) ? PDO::PARAM_INT : PDO::PARAM_STR);
        }
        $stmt->execute();
        $byTicketRows = $stmt->fetchAll(PDO::FETCH_ASSOC);

        $byService = array_map(fn(array $row): array => [
            'service_id' => (int) $row['service_id'],
            'service_name' => $row['service_name'],
            'category_name' => $row['category_name'] ?? null,
            'department_name' => $row['department_name'] ?? null,
            'count' => (int) $row['count'],
            'ministry_share' => (float) $row['ministry_share'],
            'total_revenue' => (float) $row['total_revenue'],
        ], $byServiceRows);

        $byTicket = array_map(fn(array $row): array => [
            'ticket_type' => $row['ticket_type'],
            'ticket_type_label' => ($row['ticket_type'] ?? '') === 'morning' ? 'صباحي' : 'مسائي',
            'count' => (int) $row['count'],
